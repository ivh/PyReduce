...
def arrays_to_traces(
    ...
    fiber_values: np.ndarray = None,
) -> list[Trace]:
    """Convert legacy array format to list of Traces.

    Parameters
    ----------
    trace_coeffs : np.ndarray
        Position polynomial coefficients, shape (ntrace, deg+1).
    column_range : np.ndarray
        Column ranges, shape (ntrace, 2).
    heights : np.ndarray, optional
        Extraction heights, shape (ntrace,).
    m_values : np.ndarray, optional
        Spectral order numbers, shape (ntrace,). Defaults to sequential.
    fiber_values : np.ndarray, optional
        Fiber identifiers, shape (ntrace,). Defaults to 0.
    ...
    traces : list[Trace]
        Converted traces.
    """
    ntrace = len(trace_coeffs)

    if heights is None:
        heights = np.full(ntrace, np.nan)
    if m_values is None:
        m_values = np.arange(ntrace)
    if fiber_values is None:
        fiber_values = np.zeros(ntrace, dtype=int)

    traces = []
    for i in range(ntrace):
        m = int(m_values[i]) if m_values[i] >= 0 else None
        fiber = fiber_values[i]
        height = float(heights[i]) if not np.isnan(heights[i]) else None
