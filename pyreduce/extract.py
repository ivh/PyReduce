...
class ProgressPlot:  # pragma: no cover
    def __init__(self, nrow, ncol, nslitf, title=None):
        ...
        (self.rejected_spec,) = self.ax_spec.plot([], [], ".k", ms=2, alpha=0.1)
        (self.good_spec,) = self.ax_spec.plot([], [], ".g", ms=2, alpha=0.8)
        (self.line_spec,) = self.ax_spec.plot([], "-k")

        # Slit function plot elements (rejected first as background, then good points)
        (self.rejected_slit,) = self.ax_slit.plot([], [], ".k", ms=2, alpha=0.1)
        (self.good_slit,) = self.ax_slit.plot([], [], ".g", ms=2, alpha=0.8)
