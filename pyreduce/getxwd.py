...
def getxwd(
    ...
    if nord == 1:
        ...
        if gauss:
            ...
            if pixels:
                ...
                xwd[0, 0] = (
                    pkfrac * (y1 - yym1 + 1.) / (y1 - y2)
                )  # fraction of order below central line
                xwd[0, 1] = (
                    pkfrac * (yym2 - y1 + 1.) / (y3 - y1)
                )  # fraction of order above central line
        else:
            ...
            if pixels:
                ...
                xwd[0, 0] = (
                    pkfrac * (0.5 + 0.5 * nkeep + 1) / len(prof)
                )  # fraction of order to extract
                xwd[0, 1] = (
                    pkfrac * (0.5 + 0.5 * nkeep + 1) / len(prof)
                )  # fraction of order to extract
        sig = 0.
        ...
        for iord in range(nord):
            ...
            if gauss:
                ...
                if pixels:
                    ...
                    xwd[iord, 0] = (
                        pkfrac * (y1 - yym1 + 1.) / (y1 - y2 + 1)
                    )  # fraction of order below central line
                    xwd[iord, 1] = (
                        pkfrac * (yym2 - y1 + 1.) / (y3 - y1 + 1)
                    )  # fraction of order above central line
            else:
    ...
    logging.info(
        "getxwd: extraction width (min,max) = (%d, %d)" % (np.min(xwd), np.max(xwd))
    )
