...
def test_bias(instrument, channel, files, mask):
    if len(files["bias"]) == 0:
        pytest.skip(f"No bias files for instrument {instrument}")

    bias, bhead = combine_bias(files["bias"], instrument, channel, window=50, mask=mask)
...
@pytest.mark.instrument
@pytest.mark.downloads
def test_only_one_file(instrument, channel, files, mask):
    if len(files["bias"]) == 0:
        pytest.skip(f"No bias files for instrument {instrument}")

    files = [files["bias"][0]]
    bias, bhead = combine_bias(files, instrument, channel, window=50, mask=mask)
